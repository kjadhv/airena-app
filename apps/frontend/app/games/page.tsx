...
// Assuming '@' is aliased to your 'app' directory, not the project root.
...
const GamesContent = () => {
  ...
  useEffect(() => {
    // Renamed to fetchGamesContent
    const fetchGamesContent = async () => {
      setIsLoading(true);
      try {
        // --- THIS IS THE MAIN CHANGE ---
        // Changed "sports" to "games"
        const videosQuery = query(
          collection(db, "videos"),
          where("visibility", "==", "public"),
          where("category", "==", "games") // <-- Filters for games only
        );
        // -----------------------------
        ...
        const fetchedVideos = videosSnapshot.docs.map((doc) => {
          ...
          return {
            ...
            category: data.category || "games", // This will be 'games'

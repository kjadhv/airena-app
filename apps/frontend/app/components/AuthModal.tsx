...
export default function AuthModal() {
  const { isModalOpen, loginWithGoogle, loginWithEmail, signupWithEmail, setIsModalOpen } = useAuth();
  ...
  // If the modal isn't supposed to be open, don't render it at all.
  if (!isModalOpen) {
    return null;
  }
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-[#181818] p-8 rounded-2xl shadow-lg w-full max-w-sm relative border border-white/10"
      >
        <button
          onClick={() => setIsModalOpen(false)}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-emerald-500 text-white py-3 rounded-lg font-semibold hover:bg-emerald-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
          >
